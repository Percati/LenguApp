...
1. **Nunca agregar el permiso `INTERNET` al manifiesto.** Es la prueba verificable de que la app no puede rastrear nada. Si algo parece necesitar red, no se hace.
   ...
   **Re-verificar esto — `aapt dump permissions` sobre el APK real — cada vez que se suba la versión de cualquier dependencia de AndroidX**, no solo Glance/WorkManager. Es el hallazgo más importante del proyecto hasta ahora: si WorkManager (o lo que sea que la nueva versión arrastre) cambia de comportamiento interno, `tools:node="remove"` puede dejar de alcanzar, o puede alcanzar pero romper algo en silencio — por ejemplo, el widget dejando de actualizarse sin que ningún test lo note, porque nada en este proyecto instrumenta el ciclo de vida real de WorkManager. No asumir que sigue igual: correr la verificación de nuevo.
