...
def generar(anio, idioma, nivel, banco, topics, desde=1, intentos=400, fijas=None):
    ...
    for intento in range(intentos):
        ...
        for w, (sid, order) in plan.items() if False else []:
            pass
        # Cuantas veces se uso cada topic en el anio. Elegir al azar entre los
        # candidatos repartia parejo de casualidad: al pasar de 43 a 48 semanas
        # en-B2 quedo con T08 ocho veces y T11 ninguna. Se elige entre los menos
        # usados, con azar solo para desempatar, y despues se exige cobertura.
        conteo_topic = {t: 0 for t in topics}
        for w, (tarea, topic) in fijadas.items():
            usados_topic.setdefault(tarea[0], set()).add(topic)
            conteo_topic[topic] = conteo_topic.get(topic, 0) + 1
        fallo = False
        for w in sorted(ranura):
            ...
            if not cands:
                fallo = True; break
            topic = min(cands, key=lambda t: (conteo_topic[t], rnd.random()))
